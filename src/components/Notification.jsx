...
export default function Notification() {
  ...
  /* 🔐 Track orders that already triggered shake */
  const shakenOrderIds = useRef(new Set());
  ...
  /* 🔔 SHAKE BELL — 10 SECONDS PER ORDER */
  useEffect(() => {
    let hasNewOrder = false;

    pendingOrders.forEach((order) => {
      if (!shakenOrderIds.current.has(order.id)) {
        shakenOrderIds.current.add(order.id);
        hasNewOrder = true;
      }
    });
