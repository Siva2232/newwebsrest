  ...
  const criticalProducts = useMemo(() => {
    return products
      .filter(p => {
        ...
        return !p.available || margin < 30; // low stock or margin <30%
      })
      .map(p => ({
        ...p,
        issue: !p.available ? "Out of Stock" : "Low Margin"
      }))
      .slice(0, 5);
  }, [products]);
                      ...
                      <tr key={i} className="group hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-4">
                          <span className={`px-4 py-1.5 rounded-full text-[10px] md:text-xs font-black uppercase tracking-wider ${
                            p.available ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                          }`}>
