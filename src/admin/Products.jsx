...
export default function AdminProducts() {
  ...
  const stats = [
    ...
    { label: "Live", value: products.filter(p => p.available).length, icon: CheckCircle2, color: "emerald" },
    { label: "Hidden", value: products.filter(p => !p.available).length, icon: AlertCircle, color: "rose" },
  ];
  ...
  return (
    ...
    >
      <div className={`relative bg-white rounded-[3rem] overflow-hidden border transition-all duration-700 hover:-translate-y-3 hover:shadow-2xl 
        ${product.available ? 'border-slate-100 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)]' : 'border-rose-100 shadow-none opacity-90'}
      `}>
        ...
        <div className="relative aspect-[11/13] overflow-hidden bg-slate-100">
          <img
            ...
            className={`w-full h-full object-cover transition-transform duration-[1.5s] ease-out group-hover:scale-110 
              ${!product.available && "grayscale blur-[2px] contrast-75"}`}
          />
